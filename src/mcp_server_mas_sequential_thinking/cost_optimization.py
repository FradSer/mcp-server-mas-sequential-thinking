...
class CostOptimizer:
    ...
    def __init__(
        ...
        logger.info(f"Cost optimizer initialized with {len(self.provider_profiles)} providers")
        ...
        for provider_name, profile in self.provider_profiles.items():
            ...
            if cost_env := os.getenv(cost_key):
                try:
                    ...
                    logger.info(f"Updated {provider_name} cost from environment: ${cost_env}")
                except ValueError:
                    logger.warning(f"Invalid cost value for {provider_name}: {cost_env}")
    ...
    def select_optimal_provider(
        ...
        """
        available_providers = self._get_available_providers(routing_decision.estimated_cost)
    ...
    def optimize_routing_decision(
        ...
        avg_tokens = (routing_decision.estimated_token_usage[0] + 
                     routing_decision.estimated_token_usage[1]) / 2
    ...
    def _get_available_providers(self, max_cost: float) -> List[str]:
        ...
        for provider_name, profile in self.provider_profiles.items():
            ...
            estimated_cost = max_cost * (profile.cost_per_1k_tokens / 0.0002)  # Normalize
    ...
    def _score_providers(
        ...
        for provider_name in providers:
            ...
            if self.budget_constraints.daily_limit:
                budget_utilization = self.budget_constraints.daily_spent / self.budget_constraints.daily_limit
                if budget_utilization > 0.8:  # High budget utilization
                    budget_score = 2.0 if profile.cost_tier in [CostTier.FREE, CostTier.LOW] else 0.5
    ...
    def record_usage(
        ...
        self.metrics.provider_costs[provider] = self.metrics.provider_costs.get(provider, 0.0) + actual_cost
        ...
        if provider in self.provider_profiles:
            ...
            if quality_score:
                profile.avg_quality_score = (profile.avg_quality_score * 0.9 + quality_score * 0.1)
    ...
    def get_cost_report(self) -> Dict:
        ...
        budget_status = {
            ...
            'daily_remaining': (self.budget_constraints.daily_limit or 0) - self.budget_constraints.daily_spent,
    ...
    def suggest_optimizations(self) -> List[str]:
        ...
        # Budget utilization
        if self.budget_constraints.daily_limit:
            utilization = self.budget_constraints.daily_spent / self.budget_constraints.daily_limit
            if utilization > 0.9:
                suggestions.append("Daily budget nearly exhausted - consider increasing limit or reducing usage")
            elif utilization > 0.7:
                suggestions.append("High daily budget utilization - monitor usage closely")
        ...
        if self.metrics.provider_costs:
            most_expensive = max(self.metrics.provider_costs.items(), key=lambda x: x[1])
            cheapest_available = min(
                [(p, profile.cost_per_1k_tokens) for p, profile in self.provider_profiles.items()],
                key=lambda x: x[1]
            )
            
            if most_expensive[1] > 0.01 and cheapest_available[1] == 0.0:  # Significant cost with free alternative
                suggestions.append(
