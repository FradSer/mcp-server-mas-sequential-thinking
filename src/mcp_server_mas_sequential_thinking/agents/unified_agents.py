...
class SixHatsTeamFactory:
    ...
    def create_team_by_sequence(
        ...
        if sequence_name not in sequences:
            ...
            raise ValueError(
                f"Unknown sequence '{sequence_name}'. Available: {available}"
            )
        ...
        for i, hat_color in enumerate(hat_sequence):
            agent_key = (
                f"{hat_color.value}_{i}"
                if hat_sequence.count(hat_color) > 1
                else hat_color.value
            )
        ...
        logger.info(
            f"Created Six Hats team '{sequence_name}' with {len(agents)} agents"
        )
        return agents
    ...
    def create_custom_team(
        ...
        **kwargs: Any,
        ...
        logger.info(
            f"Created custom Six Hats team '{team_name}' with {len(agents)} agents"
        )
        return agents
...
def create_hat_agent(hat_color: HatColor, model: Model, **kwargs: Any) -> Agent:
    """Create a single Six Thinking Hats agent."""
...
class UnifiedAgentFactory:
    ...
    def create_agent(self, agent_type: str, model: Model, **kwargs: Any) -> Agent:
        ...
        raise ValueError(
            f"Unknown agent type: {agent_type}. Use Six Hats: {[f'{h.value}_hat' for h in get_available_hats()]}"
        )
