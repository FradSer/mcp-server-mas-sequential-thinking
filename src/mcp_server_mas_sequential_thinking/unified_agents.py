...
class UnifiedAgentFactory:
    ...
    def __init__(self):
        self._builders = {
            ...
            for capability_name, capability in {**self.CAPABILITIES, **self.ENHANCED_ONLY_CAPABILITIES}.items()
        }
    
    def create_agent(self, agent_type: str, model: Model, enhanced_mode: bool = False, **kwargs) -> Agent:
        ...
        if agent_type in self.ENHANCED_ONLY_CAPABILITIES and not enhanced_mode:
            raise ValueError(f"Agent type '{agent_type}' is only available in enhanced mode")
    ...
    def create_team_agents(self, model: Model, team_type: str = "standard") -> Dict[str, Agent]:
        ...
        if team_type == "standard":
            ...
            for agent_type in ["researcher", "analyzer"]:
                agents[agent_type] = self.create_agent(agent_type, model, enhanced_mode=False)
            
            # Use enhanced agents for complex reasoning tasks
            for agent_type in ["planner", "critic", "synthesizer"]:
                agents[agent_type] = self.create_agent(agent_type, model, enhanced_mode=True)
                
            return agents
        else:
            raise ValueError(f"Unknown team type: {team_type}. Available: standard, enhanced, enhanced_specialized, hybrid")
...
def create_agent(agent_type: str, model: Model, enhanced_mode: bool = False, **kwargs) -> Agent:
