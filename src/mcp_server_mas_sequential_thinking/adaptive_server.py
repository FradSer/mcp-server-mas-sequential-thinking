...
class AdaptiveThoughtProcessor:
    ...
    def __init__(
        ...
        self.session_id = session_id or f"session_{uuid.uuid4().hex[:8]}_{int(datetime.utcnow().timestamp())}"
        ...
        try:
            ...
            if self.enable_adaptive_routing and self.adaptive_router:
                ...
                routing_decision = self.adaptive_router.route_thought(
                    thought_data, provider, budget_remaining
                )
                ...
                if self.enable_cost_optimization and self.cost_optimizer:
                    routing_decision, selected_provider = self.cost_optimizer.optimize_routing_decision(
            ...
            processing_metadata = {
                'strategy': routing_decision.strategy.value if routing_decision else 'standard',
                'complexity_score': routing_decision.complexity_score if routing_decision else 0.0,
                'estimated_cost': routing_decision.estimated_cost if routing_decision else actual_cost,
                'actual_cost': actual_cost,
                'token_usage': int(estimated_tokens),
                'processing_time': processing_time,
                'specialists': routing_decision.specialist_recommendations if routing_decision else [],
                'provider': selected_provider,
                'routing_reasoning': routing_decision.reasoning if routing_decision else 'Standard processing'
            }
            
            # Step 6: Record usage and store in memory
            if self.enable_cost_optimization and self.cost_optimizer and routing_decision:
            ...
            if self.enable_persistent_memory and self.memory_manager:
                ...
                try:
                    ...
                    logger.error("Failed to store error information")
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        if self.cost_optimizer:
            try:
                ...
                summary['optimization_suggestions'] = self.cost_optimizer.suggest_optimizations()
            except Exception as e:
...
def create_adaptive_processor(
    ...
    if enable_cost_optimization is None:
        enable_cost_optimization = bool(
            os.getenv("DAILY_BUDGET_LIMIT") or 
            os.getenv("MONTHLY_BUDGET_LIMIT") or
            os.getenv("PER_THOUGHT_BUDGET_LIMIT")
        )
