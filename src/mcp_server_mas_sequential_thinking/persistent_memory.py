...
from sqlalchemy import (
    ...
    Boolean, Float, JSON, ForeignKey, Index, desc
)
...
class SessionRecord(Base):
    ...
    thoughts = relationship("ThoughtRecord", back_populates="session", cascade="all, delete-orphan")
...
class PersistentMemoryManager:
    ...
    def store_thought(
        ...
        with self.SessionLocal() as db:
            ...
            if processing_metadata:
                thought_record.processing_strategy = processing_metadata.get('strategy')
                thought_record.complexity_score = processing_metadata.get('complexity_score')
                thought_record.estimated_cost = processing_metadata.get('estimated_cost')
                thought_record.actual_cost = processing_metadata.get('actual_cost')
                thought_record.token_usage = processing_metadata.get('token_usage')
                thought_record.processing_time = processing_metadata.get('processing_time')
                thought_record.specialist_used = processing_metadata.get('specialists', [])
                thought_record.processed_at = datetime.utcnow()
            
            db.add(thought_record)
            
            # Update session stats
            session_record.total_thoughts += 1
            session_record.updated_at = datetime.utcnow()
            if processing_metadata and processing_metadata.get('actual_cost'):
                session_record.total_cost += processing_metadata['actual_cost']
            
            # Handle branching
            if thought_data.branch_id:
                branch_record = db.query(BranchRecord).filter(
                ...
                branch_record.thought_count += 1
            
            db.commit()
            return thought_record.id
    ...
    def get_session_thoughts(
        ...
        with self.SessionLocal() as db:
            query = db.query(ThoughtRecord).filter(
    ...
    def get_thought_by_number(
        ...
        with self.SessionLocal() as db:
            return db.query(ThoughtRecord).filter(
                ThoughtRecord.session_id == session_id,
    ...
    def get_branch_thoughts(
        ...
        with self.SessionLocal() as db:
            ...
            ).order_by(ThoughtRecord.thought_number).all()
    ...
    def prune_old_sessions(
        ...
        with self.SessionLocal() as db:
            ...
            old_sessions = db.query(SessionRecord).filter(
                SessionRecord.updated_at < cutoff_date
            ).order_by(desc(SessionRecord.updated_at)).offset(keep_recent)
    ...
    def get_usage_stats(
        ...
        with self.SessionLocal() as db:
            ...
            # Thought stats
            thought_stats = db.query(ThoughtRecord).filter(
                ThoughtRecord.created_at >= cutoff_date
            ).all()
            
            total_thoughts = len(thought_stats)
            total_cost = sum(t.actual_cost or 0 for t in thought_stats)
            total_tokens = sum(t.token_usage or 0 for t in thought_stats)
            avg_processing_time = sum(t.processing_time or 0 for t in thought_stats) / max(total_thoughts, 1)
    ...
    def record_usage_metrics(
        ...
        with self.SessionLocal() as db:
            ...
            if existing:
                ...
                existing.avg_processing_time = (
                    (existing.avg_processing_time * (existing.thought_count - thought_count) + 
                     processing_time * thought_count) / existing.thought_count
                )
                if not success:
                    existing.success_rate = (existing.success_rate * (existing.thought_count - thought_count)) / existing.thought_count
            else:
                # Create new record
...
def create_persistent_memory(database_url: Optional[str] = None) -> PersistentMemoryManager:
