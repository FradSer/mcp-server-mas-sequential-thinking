...
# Quick test function
def test_routing_with_examples():
    ...
    for i, thought_text in enumerate(test_cases, 1):
        ...
        decision = router.route_thought(thought_data)
        print(f"Test {i}: {thought_text[:50]}...")
        print(f"  → {decision.strategy.name} ({decision.strategy.estimated_time_seconds}s)")
        print(f"  → Complexity: {decision.complexity_metrics.complexity_score:.1f}")
        print()
